...
`format_transform("fakeg")` follows the general transform default `frameID=-1`;
pass `frameID="all"` when full-file Gaussian-like rendering is required.

| Capability | Support | Rendered content | Boundaries |
| ---------- | ------- | ---------------- | ---------- |
| <!-- feature-area:File-level Gaussian-like writer -->File-level Gaussian-like writer | Partial | `.fakeg` files from parsed Gaussian output data. | File-level rendering only; frame-level fakeG writing is not provided, the selected frames follow `frameID`, and byte-for-byte Gaussian log reproduction is not claimed. |
