...
完整的 File/Frame 类层级、source span、解析诊断和 renderer 约束见 [Parser 契约](parser-contract.md)。本页只说明如何把一个 codec 接入 registry。
...
示例中的无约束依赖只用于展示最小包结构。发布插件前，必须把它替换成实际测试过的 MolOP 版本范围。开发版文档可能领先于 PyPI；若插件依赖顶部横幅标记为未发布的接口，开发阶段应固定横幅中的 commit，等接口进入正式版本后再发布插件并设置依赖下限。详见[文档版本](../versioning.md)。
...
仓库内置 codec 还需要同步更新中英文格式页和用户示例，并运行[质量门禁](quality.md)。不要在文档中维护第二份格式清单；能力状态由 `tests/format_feature_coverage/support_matrix.py` 和格式页生成区段维护。
