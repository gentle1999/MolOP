...
| 能力 | 支持程度 | 可渲染内容 | 边界 |
| ---- | -------- | ---------- | ---- |
| <!-- feature-area:File-level Gaussian-like writer -->文件级 Gaussian-like writer | 部分支持 | 从已解析 Gaussian 输出数据写出 `.fakeg` 文件。 | 只支持文件级渲染；不提供帧级 fakeG writer，也不保证逐字复现 Gaussian log。 |
