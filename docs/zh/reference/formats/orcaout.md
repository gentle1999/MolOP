...
| <!-- feature-area:Charge and spin populations -->Charge and spin populations | 部分支持 | 解析已有 ORCA 输出中的闭壳层/开壳层 Mulliken、Loewdin 原子 charge/spin 表及 Hirshfeld charge/spin 表。 | Reduced orbital population、Mayer valence/键级表和测试 fixture 外的方案仍未结构化。 | `tests/test_orca_output_fixtures.py::test_orca_output_structured_parse_contract`<br>`tests/test_orca_output_fixtures.py::test_orca_closed_shell_population_schemes_are_all_structured`<br>`tests/test_orca_output_fixtures.py::test_orca_open_shell_mulliken_and_lowdin_charge_spin_tables_are_structured` |
