...
LastEditors: TMJ
LastEditTime: 2025-12-15 00:34:17
Description: 请填写简介
"""
        ...
        if template:
            if os.path.exists(template) and os.path.isfile(template):
                        ...
                        return "\n--Link1--\n".join(
                            [
                                self._render_one_frame(
                                    ...
                                    suffix=frame.suffix,
                                    chk=f"chk_{frame.frame_id + 1}_{chk}"
                                    if chk
                                    else None,
                ...
                raise FileNotFoundError(
                    f"template file {template} not found or not a file"
                )
        else:
            return self._render_one_frame(
                options=options,
        ...
        if link1_mode:
            ...
            if isinstance(route_params.get("geom"), str):
                ...
                if any(
                    ...
                    for key in route_params.get("geom").keys()  # type: ignore
                ):
                    print_coords = False
                if any(
...
class GJFFileFrameDisk(
    DiskStorageMixin, GJFFileFrameMixin, BaseCoordsFrame["GJFFileFrameDisk"]
):
    def _render(
        self,
        options: str | None = None,
