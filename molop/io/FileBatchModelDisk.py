...
class FileBatchModelDisk(MutableMapping):
    ...
    __diskfiles: OrderedDict[str, FileDiskType]

    def __init__(self, diskfiles: Iterable[FileDiskType] | None = None):
        self.__diskfiles: OrderedDict[str, FileDiskType] = OrderedDict()
    ...
    def __sub__(
        self, other: Union["FileBatchModelDisk", Iterable[str]]
    ) -> "FileBatchModelDisk":
        """
        ...
        if isinstance(other, FileBatchModelDisk):
            ...
            return NotImplemented
        filtered_files = [
            f for f in self.__diskfiles.values() if f.file_path not in other_paths
        ]
    ...
    def __and__(
        ...
        if isinstance(other, FileBatchModelDisk):
            ...
            return NotImplemented
        common_files = [
            f for f in self.__diskfiles.values() if f.file_path in other_paths
        ]
    ...
    def filter_state(
        ...
        def judge_func(diskfile: FileDiskType) -> bool:
            ...
            if state == "ts":
                ...
                if any(
                    frame.geometry_optimization_status is not None for frame in diskfile
                ):
                    return (
        ...
        desc = (
            f"Filtering {state} files {'negated' if negate else ''} with {n_jobs} jobs"
        )
        mask = self._parallel_execute(judge_func, desc, n_jobs)
        filtered_files = [
            diskfile for diskfile, keep in zip(self, mask, strict=True) if keep
        ]
        return self.new_batch(filtered_files)
    ...
    def filter_custom(
        self,
        condition: Callable[[FileDiskType], bool],
        n_jobs: int = 1,
    ) -> "FileBatchModelDisk":
        """
        ...
        Parameters:
            condition (Callable[[FileDiskType], bool]): A function that accepts a diskfile object and returns True (keep) or False (discard).
    ...
    def groupby(
        self, key_func: Callable[[FileDiskType], str], n_jobs: int = 1
    ) -> dict[str, "FileBatchModelDisk"]:
        """
        Group the files into multiple batches based on a key function.

        Parameters:
            key_func (Callable[[FileDiskType], str]): A function that generates a group key for each file.
        ...
        temp_groups: dict[str, List[FileDiskType]] = {}
        for diskfile, key in zip(self, keys, strict=True):
            if key not in temp_groups:
                temp_groups[key] = []
            temp_groups[key].append(diskfile)
        return {
            group_key: self.new_batch(files) for group_key, files in temp_groups.items()
        }
    ...
    def format_transform(
        ...
        **kwargs,
        ...
        def transform_func(diskfile: FileDiskType) -> tuple[str, str | List[str]]:
    ...
    def to_summary_df(
        ...
        **kwargs,
        ...
        def process_file_summary(diskfile: FileDiskType) -> List[pd.Series]:
            if mode == "file":
    ...
    def draw_grid_image(
        ...
        **kwargs,
        ...
        for file_path, smi in (
            self[:maxMols]
            .format_transform("smi", output_dir=None, n_jobs=n_jobs)
            .items()
        ):
