...
def clean_neighbor_radicals(
    ...
    for radical_atom_1, radical_atom_2 in list(
        ...
        if (
            ...
            """
            This code block checks if two radical atoms are bonded and if they have a valence greater than their valence.
...
def break_one_bond(
    ...
    if (
        ...
        while res := smarts.findall(omol):
            idxs = res.pop(0)
            logger.debug(f"{DEBUG_TAG} break bond {idxs[0]} and {idxs[1]}")
...
def fix_under_bonded_dipole(omol: pybel.Molecule):
    ...
    while len(res):
        logger.debug(f"{DEBUG_TAG} fixing under bonded dipole")
        idxs = res.pop(0)
...
def clean_resonances_9(omol: pybel.Molecule) -> pybel.Molecule:
    ...
    while len(res):
        logger.debug(f"Cleaning splited charges")
        idxs = res.pop(0)
    ...
    *This function can not be used separately.*
    ...
    if given_charge >= 0:
        ...
        while res := smarts.findall(omol):
            ...
            logger.debug(
                f"{DEBUG_TAG} Fix fake dipole: {idxs[0]} - {idxs[1]}, charge to be allocated: {given_charge}"
            )
    return given_charge
...
def fix_CN_in_doubt(given_charge: int, omol: pybel.Molecule):
    ...
    *This function can not be used separately.*
    ...
    Parameters:
        ...
        omol (OBMol): The molecule object.
    """
    smarts = pybel.Smarts("[#6v4+0]=,#[#7v4+0]")
    doubt_pair = [
    ...
    *This function can not be used separately.*
    ...
    if given_charge > 0:
        ...
        while res := smarts.findall(omol):
            ...
            logger.debug(
                f"{DEBUG_TAG} Fix over-bonded N: {idxs[0]}, charge to be allocated: {given_charge}"
            )
    return given_charge
    ...
    while res := smarts.findall(omol):
        ...
        for neighbour_atom in ob.OBAtomAtomIter(atom):
            if atom.GetDistance(neighbour_atom) >= 1.1 * (
                ...
                logger.debug(
                    f"{DEBUG_TAG} Fix un-convinced possitive N: {idxs[0]}, charge to be allocated: {given_charge}"
                )
                break
        else:
            ...
            given_charge -= 1
            logger.debug(
                f"{DEBUG_TAG} Fix convinced possitive N: {idxs[0]}, charge to be allocated: {given_charge}"
            )
    return given_charge
...
def final_check(omol: pybel.Molecule, total_charge: int = 0, total_radical: int = 0):
    ...
    logger.debug(f"{DEBUG_TAG} Now SMILES: {omol.write('smi')}")
    charge, radical = 0, 0
    for atom in omol.atoms:
...
def rdmol_check(
    ...
    for atom in rdmol.GetAtoms():
        ...
        if is_metal(atom.GetAtomicNum()):
            has_metal = True
    logger.debug(
        f"{DEBUG_TAG} check rdmol, target charge: {total_charge} charge: {charge}, target radical: {total_radical} radical: {radical}"
    )
    if has_metal:
...
def omol_to_rdmol(
    ...
    logger.debug("try tranform by mol2")
    ...
    if rdmol is not None:
        ...
        if rdmol_check(rdmol, total_atom_num, total_charge, total_radical):
            return rdmol
    logger.debug("try tranform by cdxml")
    ...
    logger.debug("try tranform by graph")
    if rdmol is not None:
        rdmol = Chem.AddHs(rdmol)
        if rdmol_check(rdmol, total_atom_num, total_charge, total_radical):
            return rdmol
    logger.debug("try tranform by sdf")
