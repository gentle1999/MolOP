...
def find_rigid_transform(P: np.ndarray, Q: np.ndarray, tolerance: float = 1e-6) -> np.ndarray:
    """
    ...
    if not np.isclose(det_r, 1.0, atol=tolerance):
        raise ValueError(f"Computed rotation matrix implies scaling/reflection. Det: {det_r}")
    ortho_check = R @ R.T
...
def verify_transform_reversibility(
    ...
    return reconstruction_error < reconstruction_error_tolerance
...
def invert_transform_coords(orientation_coords: np.ndarray, T: np.ndarray) -> np.ndarray:
