...
LastEditors: TMJ
LastEditTime: 2025-12-16 12:24:48
...
class SPMSCalculator(BaseModel):
    ...
    anchor_list: Sequence[int] | None = Field(
        default=None,
    ...
    sphere_radius: float | None = Field(
        default=None,
    ...
    def radius_list(self) -> np.ndarray:
        ...
        if self.atom_radius == "vdw":
            return np.array([pt.GetRvdw(atom.GetAtomicNum()) for atom in self.rdmol.GetAtoms()])
    ...
    def SPMS(self) -> np.ndarray:
        ...
        all_cross = np.array(
            [np.cross(atom_vec.reshape(-1, 3), mesh_xyz, axis=1) for atom_vec in positions]
        ).transpose(1, 0, 2)
...
def geometry_initialize(
    ...
    *,
    ...
    dis_list = np.linalg.norm(rwmol.GetConformer().GetPositions() - np.array(center), axis=1)
    farthest_atom = np.argmax(dis_list)
    dis_list[np.argmin(dis_list)] = np.max(dis_list)
    nearest_atom = np.argmin(dis_list)
    moloplogger.debug(f"{DEBUG_TAG}: Nearest atom: {nearest_atom}, Farthest atom: {farthest_atom}")
...
def get_default_sphere_radius(rdmol: Chem.rdchem.Mol) -> float:
    ...
    """
    atom_radius_vector = np.array([pt.GetRvdw(atom.GetAtomicNum()) for atom in rdmol.GetAtoms()])
