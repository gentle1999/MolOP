...
def build_mol_from_atoms_and_bonds(
    ...
    *,
    ...
    for bond_indexes in bonds:
        ...
        mol.AddBond(beginAtomIdx=begin_atom_idx, endAtomIdx=end_atom_idx, order=bond_type)
...
def get_sub_mol(origin_mol: RdMol, scale: list[int]) -> RdMol:
    ...
    formal_num_radicals = [origin_mol.GetAtomWithIdx(idx).GetNumRadicalElectrons() for idx in scale]
    bonds = [
        (
            bond.GetBeginAtomIdx(),
            bond.GetEndAtomIdx(),
...
def transform_replacement_index(
    ...
    *,
    ...
    if bond_tag in (Chem.rdchem.BondType.SINGLE,):
        ...
        radical_idxs = [
            atom.GetIdx() for atom in mol.GetAtoms() if atom.GetNumRadicalElectrons() >= 2
        ]
        if absolute_idx is None:
            if relative_idx >= len(radical_idxs):
                raise ValueError("Relative index is out of range.")
        ...
        radical_idxs = [
            atom.GetIdx() for atom in mol.GetAtoms() if atom.GetNumRadicalElectrons() >= 3
        ]
        if absolute_idx is None:
            if relative_idx >= len(radical_idxs):
                raise ValueError("Relative index is out of range.")
    ...
    *,
    ...
    if start_idx and end_idx:
        if (
            ...
            raise ValueError("start_idx and end_idx should be bonded but not in a ring.")
    else:
    ...
    if bond_tag in (
        ...
        for atom in origin_mol.GetAtomWithIdx(start).GetNeighbors():
            if atom.GetIdx() != end:
                ...
                GeometryTransformation.standard_orient(origin_mol, [start, end, neighbor_idx])
                break
        else:
            GeometryTransformation.standard_orient(origin_mol, [start, end])
        skeleton, new_start = get_skeleton(origin_mol=origin_mol, start=start, end=end)
    ...
    mapping = atom_idxs[replacement.GetNumAtoms() :] + atom_idxs[: replacement.GetNumAtoms()]
...
def combine_skeleton_replacement(
    skeleton: Chem.RWMol | RdMol,
    replacement: Chem.RWMol | RdMol,
    start: int,
    bond_tag: Chem.rdchem.BondType,
):
...
def build_replacement(
    ...
    moloplogger.debug(f"{DEBUG_TAG} | Replacement initialization of structure replacement passed")
...
def reset_atom_index(mol: Chem.rdchem.Mol, mapping: Sequence[int]) -> Chem.rdchem.Mol:
    ...
    """
    # new index
    new_idx = list(mapping) + [idx for idx, atom in enumerate(mol.GetAtoms()) if idx not in mapping]
...
def make_dative_bonds(rwmol: Chem.rdchem.RWMol, ratio=1.3) -> Chem.rdchem.RWMol:
    ...
    metal_atoms = [atom.GetIdx() for atom in rwmol.GetAtoms() if is_metal(atom.GetAtomicNum())]
    datived_rwmol = []
    for shuffled_metal_atoms in itertools.permutations(metal_atoms, len(metal_atoms)):
        ...
        for metal_atom in shuffled_metal_atoms:
            ...
            negative_atoms = [
                ...
                * estimate_bond_length(
                    ...
                    bond_list[abs(temp_rwmol.GetAtomWithIdx(atom.GetIdx()).GetFormalCharge())],
                )
            ]
            ...
            for dative_atom in dative_atoms:
                if not temp_rwmol.GetBondBetweenAtoms(metal_atom, dative_atom):
                    if temp_rwmol.GetConformer().GetAtomPosition(metal_atom).Distance(
                        ...
                        neighbour_atoms = [
                            neighbor.GetIdx()
                            for neighbor in temp_rwmol.GetAtomWithIdx(dative_atom).GetNeighbors()
                        ]
                        temp_rwmol.AddBond(dative_atom, metal_atom, Chem.BondType.DATIVE)
                        # avoid unusual angle
        ...
        moloplogger.debug(f"{DEBUG_TAG} | Remained negative atoms: {remained_negative_atoms}")
        for remained_negative_atom in remained_negative_atoms:
            metal_atoms = [
                atom.GetIdx() for atom in temp_rwmol.GetAtoms() if is_metal(atom.GetAtomicNum())
            ]
            if not metal_atoms:
                continue
            metal_atoms.sort(
                key=lambda x: (
                    temp_rwmol.GetConformer()
                    .GetAtomPosition(x)
                    .Distance(temp_rwmol.GetConformer().GetAtomPosition(remained_negative_atom))
                )
            )
            distance = (
                temp_rwmol.GetConformer()
            ...
            if (
                ...
                temp_rwmol.AddBond(
                    ...
                    metal_atoms[0],
                    bond_list[-temp_rwmol.GetAtomWithIdx(remained_negative_atom).GetFormalCharge()],
                )
