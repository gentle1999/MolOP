...
def translate_anchor(pos: np.ndarray, idx: int | Sequence[int]) -> np.ndarray:
    if isinstance(idx, (int, np.integer)):
        return pos - pos[idx]
    elif isinstance(idx, Sequence):
        for i in idx:
            assert isinstance(i, (int, np.integer)), "idx must be int or sequence of int"
        return pos - np.mean(pos[idx], axis=0)
    else:
        raise TypeError("idx must be int or sequence of int")


def translate_mol_anchor(mol: RdMol, idx: int | Sequence[int], conformer_id=0):
    """
...
def rotate_anchor_to_axis(
    pos: np.ndarray, idx: int | Sequence[int], axis: Literal["x", "y", "z"]
) -> np.ndarray:
    if isinstance(idx, (int, np.integer)):
        point = pos[idx]
    elif isinstance(idx, Sequence):
...
def rotate_mol_anchor_to_plane(
    ...
    set_conformer_position(
        mol,
        rotate_anchor_to_plane(mol.GetConformer(conformer_id).GetPositions(), idx, plane),
        conformer_id,
    )
